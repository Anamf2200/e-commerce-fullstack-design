...
const AdminPanel = ( ) => {
  ...
  const [deleteProduct, { isLoading: deleting }] = useDeletedProductMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await updateProduct({ id: editingId, updatedProduct: data }).unwrap();
        setEditingId(null);
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    const handleLogout = () => {
    dispatch(logOut()); // clears state and localStorage
    ...
    <div className="min-h-screen bg-gray-100 p-4 md:p-6">
      ...
      <div className="bg-white p-4 md:p-6 rounded-lg shadow-md mb-6 md:mb-8 max-w-2xl mx-auto">
        ...
        <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
          ...
          >
            {editingId ? updating ? "Updating..." : "Update Product" : adding ? "Adding..." : "Add Product"}
          </button>
        </form>
      </div>
